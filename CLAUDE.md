...
- **Public releases are a separate pipeline from CI.** `.github/workflows/release.yml` (triggered by a `vX.Y.Z` tag) builds a universal, Developer-ID-signed, notarized `Keelhaven-<version>.dmg` and attaches it to a GitHub Release — distinct from `ci.yml`'s ad-hoc per-arch zips, which only feed `Scripts/install-latest.sh` for personal installs. It needs Apple Developer secrets that don't exist in the repo yet; see `docs/RELEASING.md` before the first tag push. `Scripts/make-dmg.sh` (`make dmg`) packages any built `.app` into a DMG and works locally without those secrets.
