...
The public website + docs (VitePress, English default, Chinese planned) live in `site/`. Preview locally with `npm --prefix site run dev`. On pushes to main that touch `site/**`, `.github/workflows/website.yml` builds the static output and pushes it to the public `keelapps/keelhaven.app` repo, served by GitHub Pages at **https://keelhaven.app** — doc *sources* live in this repo under FSL-1.1-MIT (see `LICENSE.md`), and the rendered site keeps its own © Keelapps notice. Site changes are ignored by the expensive macOS CI. `docs/WEBSITE.md` documents the whole path from source to live domain.
