...
- **Deliberately not built yet** (don't add speculatively): file-level browsing inside snapshots (whole-snapshot restore is shipped: plan actions → Restore…), retention/prune, extra backends, launchd scheduling, sandboxing. The canonical list lives in `docs/ARCHITECTURE.md` § "Not yet built (deliberately)" — keep the two in sync.
